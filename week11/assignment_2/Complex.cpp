...
Complex::Complex() {
     ...
     */
   std::pair<float, float> val = std::make_pair(0,0);
   this->value = val;
...
Complex::Complex(float _val1, float _val2) {
     ...
     */
   std::pair<float, float> val = std::make_pair(_val1, _val2);
   this->value = val;
}
...
Number *Complex::add(Number * num) {
	...
	this->set_val(this->real() + other->real(), this->imag() + other->imag());
	delete(other);
	return this;
...
Number *Complex::mul(Number * num) {
	...
	Complex * other = dynamic_cast<Complex*>(num);
	this->set_val(this->real() * other->real() - this->imag() * other->imag(), this->imag() * other->imag() + this->imag() * other->real());
	delete(other);
	return this;
...
void Complex::set_val(double real, double imag) {
     ...
     * 이 때 make_pair함수를 이용
     */
    std::pair<double, double> val = std::make_pair(real, imag);
    this->value = val;
