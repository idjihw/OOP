...
Number *Integer::add(Number * num) {
	...
	other->set_val(this->value + other->val());
	delete(this);
	return other;	

    } else if (num->types() == COMPLEX) {
	Complex * other = dynamic_cast<Complex*>(num);
	other->set_val(this->value + other->real(), other->imag());
	delete(this);
	return other;
...
Number *Integer::mul(Number * num) {
	...
	Complex * other = dynamic_cast<Complex*>(num);
	other->set_val(this->value * other->real(), this->value * other->imag());
	delete(this);
	return other;
