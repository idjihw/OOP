...
int main()
{
	...
	bbuyo_obj->set_next_block(generate_next_block());	// 판에 바로 올릴 블록 생성
	big_block bb = bbuyo_obj->set_next_block();			// 올린 블록을 bb에 정의
	bbuyo_obj->set_next_block(generate_next_block());	// 다음 블록을 나타내주는 칸에도 똑같이 생성
	bbuyo_obj->print();
